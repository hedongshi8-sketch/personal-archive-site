...
- Supabase Auth 邮件：先用 Gmail 免费 SMTP 跑通注册确认/重发确认邮件，后续可换自有域名，配置看 [docs/supabase-smtp.md](docs/supabase-smtp.md)
...
`verify:email` 只做安全的基础体检，默认不会真的发邮件。最方便的最终收件箱验证跑 `npm run verify:email:guided`：它会在当前终端临时读取 Gmail App Password，不写入文件，然后发送一封 Gmail SMTP 测试邮件，并触发一次 Supabase 密码重置邮件。已经在当前终端或 `.env.local` 配好 `GMAIL_APP_PASSWORD` 时，也可以直接跑 `npm run verify:email:final`。
