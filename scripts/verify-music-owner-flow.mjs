...
assertIncludes(appSource, "playbackMessage", "music player exposes direct playback feedback");
assertIncludes(appSource, "player-feedback", "music player renders feedback inside player surface");
assertIncludes(appSource, "还没有上传音频文件", "music play click explains tracks without audio");
assertIncludes(appSource, "公开歌单还在同步，稍等几秒再点播放。", "music play click explains loading state");
assert(!musicSectionSource.includes("disabled={!activeTrack && !isPlaying}"), "music play button is never silently disabled");
assertIncludes(appSource, "handlePlaybackPointerDown", "music playback starts from pointer gesture when possible");
assertIncludes(appSource, "requestTrackPlayback(audio)", "music playback request is shared by pointer and click handlers");
assertIncludes(appSource, "controls={playbackState === \"error\"}", "music player exposes native controls after playback failure");
assertIncludes(appSource, "已显示浏览器原生播放条", "music playback failure explains native controls fallback");
...
assertIncludes(appSource, "src={activeTrack?.audioUrl}", "active music track uses selected track audio URL");
assertIncludes(appSource, "preload=\"auto\"", "active music track preloads for responsive playback");
...
assertIncludes(stylesSource, ".player-feedback", "music player feedback style exists");
assertIncludes(stylesSource, ".player-audio-element:not([controls])", "native audio control stays hidden until fallback");
